...
def band(value: dict[str, Any]) -> str:
    return f"{percent(value.get('rate'))} [{percent(value.get('low'))}, {percent(value.get('high'))}]"
...
def _environment(root: Path) -> Environment:
    ...
    environment.filters.update(number=number, percent=percent, interval=interval, band=band)
...
def render_manifest(manifest: dict[str, Any], root: Path = ROOT) -> dict[str, str]:
    ...
    page = _environment(root).get_template("readouts/templates/readout.html.jinja").render(
...
def build_bundle(repository: EvidenceRepository) -> dict[str, Any]:
    experiments = [repository.latest_manifest(row["key"]) for row in repository.list_experiments()]
    if not experiments:
    ...
    return {
        "schema_version": "1", "generated_at": calibration.get("computed_at", calibration.get("generated_at")),
        "experiments": experiments, "calibration": calibration,
...
def publish(repository: EvidenceRepository, root: Path = ROOT, write: bool = False) -> list[str]:
    ...
    rendered[root / "web/public/results/bundle.json"] = json.dumps(bundle, indent=2, sort_keys=True, allow_nan=False) + "\n"
    errors = []
    for path, expected in rendered.items():
        ...
        if write:
            ...
            differences = list(difflib.unified_diff(actual.splitlines(), expected.splitlines(), fromfile=str(path), tofile="stored-record render", lineterm=""))
            errors.append("\n".join(differences[:35]))
    return errors
