...
def adjust(
    ...
    return {
        ...
        "variance_reduction_percent": float(100 * reduction),
        "equivalent_sample_multiplier": float(1 / (1 - reduction)) if reduction < 1 else None,
        "equivalent_sample_saved": float((len(c) + len(t)) * reduction),
        "n_saved": float((len(c) + len(t)) * reduction),
        "method": "pooled theta = covariance(pre, post) / variance(pre)",
        "limitation": "Same-sample theta has small finite-sample bias; pre-period covariates must be unaffected by treatment.",
    }
