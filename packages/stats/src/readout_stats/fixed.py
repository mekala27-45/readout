...
def from_summary(
    mean_c: float, mean_t: float, var_c: float, var_t: float,
    n_c: int, n_t: int, alpha: float = 0.05, kind: str = "mean",
) -> dict[str, Any]:
    ...
    critical = float(stats.t.ppf(1 - alpha / 2, df)) if kind == "mean" and df else float(stats.norm.ppf(1 - alpha / 2))
    if kind == "proportion":
        ...
        test_se = se
    if test_se:
        p = float(2 * (stats.t.sf(abs(estimate / test_se), df) if kind == "mean" and df else stats.norm.sf(abs(estimate / test_se))))
    else:
        p = 1.0 if estimate == 0 else 0.0
    relative = mean_t / mean_c - 1 if mean_c != 0 else None
    relative_se = float(np.hypot(np.sqrt(vt) / abs(mean_c), abs(mean_t / mean_c) * (np.sqrt(vc) / abs(mean_c)))) if mean_c != 0 else None
    if relative is not None and relative_se is not None and (not np.isfinite(relative) or not np.isfinite(relative_se)):
        relative = relative_se = None
    relative_critical = float(stats.norm.ppf(1 - alpha / 2))
    return {
        "estimate": float(estimate), "ci_low": float(estimate - critical * se),
        "ci_high": float(estimate + critical * se), "relative": relative,
        "relative_ci_low": relative - relative_critical * relative_se if relative is not None and relative_se is not None else None,
        "relative_ci_high": relative + relative_critical * relative_se if relative is not None and relative_se is not None else None,
        "relative_se": relative_se, "p_value": p, "se": se,
        "control_mean": float(mean_c), "treatment_mean": float(mean_t),
        "n_control": int(n_c), "n_treatment": int(n_t), "df": float(df) if df else None,
        "kind": kind, "alpha": alpha,
        "method": {"mean": "Welch t", "proportion": "pooled two-sample z; unpooled Wald interval", "ratio": "unit delta method"}[kind],
    }


def compare(control: ArrayLike, treatment: ArrayLike, kind: str = "mean", alpha: float = 0.05) -> dict[str, Any]:
...
def bootstrap(control: ArrayLike, treatment: ArrayLike, alpha: float = 0.05, seed: int = 20260922, draws: int = 400) -> dict[str, Any]:
    c, t = samples(control), samples(treatment)
    ...
    return {"estimate": float(t.mean() - c.mean()), "ci_low": float(low), "ci_high": float(high), "draws": draws, "seed": seed, "method": "unit percentile bootstrap"}
