...
def fingerprint(rows: list[dict[str, Any]]) -> str:
    payload = sorted(rows, key=lambda row: str(row.get("unit_id", "")))
...
def metric_available(row: dict[str, Any], metric: dict[str, Any]) -> bool:
    value = row.get("metrics", {}).get(metric["key"])
    if value is None or not np.isfinite(float(value)):
        return False
    if metric.get("kind") == "ratio":
        denominator = row.get("denominators", {}).get(metric["key"])
        return (
            denominator is not None
            and bool(np.isfinite(float(denominator)))
...
def srm(observed: dict[str, int], allocation: dict[str, float] | None = None) -> dict[str, Any]:
    ...
    if (
        len(allocation) < 2
        or set(observed) != set(allocation)
        or any(v < 0 for v in observed.values())
        or any(v <= 0 for v in allocation.values())
        or not np.isclose(sum(allocation.values()), 1)
    ):
    ...
    if total == 0:
        return {
            ...
            "detail": "no data",
            "observed": observed,
            "expected": {k: 0 for k in allocation},
            "p_value": None,
            "statistic": None,
            "conventional_verdict": "block",
    ...
    return {
        ...
        "expected": expected,
        "short_arm": short,
        "short_by": float(shortage),
        "detail": f"{short} is short by {shortage:.1f} units versus configured allocation; block below 0.001, warn below 0.05.",
        "conventional_verdict": "warn" if p < 0.05 else "pass",
        "blocking_verdict": "block" if p < 0.001 else "pass",
...
def check_health(design: dict[str, Any], rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    design_digest = design_fingerprint(design)
    ...
    coverage = {
        name: exposed[name] / assigned[name] if assigned[name] else 0.0 for name in allocation
    }
    ...
    records.append(
        {
            "check": "exposure",
            "scope": "assigned",
            "status": "warn" if fraction < 0.9 else "pass",
            "fraction": fraction,
            "by_arm": coverage,
            "detail": "Analysis conditions on exposure. Random assignment alone does not identify this effect if treatment changes exposure.",
            "data_digest": digest,
            "design_digest": design_digest,
        }
    )
    for metric in design.get("metrics", []):
        missing = {
            name: sum(
                ...
                for r in rows
                if r["variant"] == name
                and r.get("exposed", True)
                and not metric_available(r, metric)
            )
            for name in allocation
        }
        fractions = {
            name: missing[name] / exposed[name] if exposed[name] else 1.0 for name in allocation
        }
        required = metric["key"] == design.get("primary_metric_key") or metric["key"] in design.get(
        ...
        records.append(
            {
                ...
                "status": "block"
                if required and empty_arm
                else "warn"
                if max(fractions.values()) - min(fractions.values()) > 0.01
                or max(fractions.values()) > 0.05
                else "pass",
                "by_arm": fractions,
                "missing": missing,
                "detail": "Missing metric values or ratio denominators by exposed arm; primary and guardrail metrics require two valid units per arm. Complete-case analysis assumes ignorable missingness.",
                "data_digest": digest,
                "design_digest": design_digest,
            }
        )
