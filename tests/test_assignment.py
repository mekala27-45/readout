...
EXPERIMENT = {"key": "demo", "salt": "salt-one", "variants": [
    {"name": "control", "allocation": .5}, {"name": "treatment", "allocation": .5}]}
...
def test_assignment_is_uniform():
    counts = np.bincount([assign(EXPERIMENT, f"unit-{i}")["bucket"] for i in range(100000)], minlength=10000)
    assert chisquare(counts).pvalue > .001
...
def test_salt_decorrelates_experiments():
    ...
    assert abs(float(np.corrcoef(first, second)[0, 1])) < .04


def test_ramp_is_stable():
    ramp = {**EXPERIMENT, "variants": [
        {"name": "control", "allocation": .9}, {"name": "treatment", "allocation": .1}]}
...
def test_assignment_is_deterministic_across_processes():
    ...
    first = subprocess.run([sys.executable, "-c", code], capture_output=True, text=True, check=True, env=os.environ, timeout=30)
    second = subprocess.run([sys.executable, "-c", code], capture_output=True, text=True, check=True, env=os.environ, timeout=30)
