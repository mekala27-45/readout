...
def test_sequential_false_positives_with_monte_carlo_tolerance(calibration):
    for key in ["peeking", "peeking_proportion"]:
        ...
        assert final["sequential"]["low"] <= .05
        assert final["naive"]["low"] > .05
...
def test_exposure_dilution(calibration):
    ...
    assert row["assigned"]["ci_low"] <= .24 <= row["assigned"]["ci_high"]
    assert row["exposed"]["ci_low"] <= .4 <= row["exposed"]["ci_high"]
