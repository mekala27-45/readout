...
@pytest.mark.parametrize("updates", [
    ...
    {"variants": [{"name": "control", "allocation": .3}, {"name": "treatment", "allocation": .3}]},
    ...
    {"variants": [{"name": "control", "allocation": .5}, {"name": "control", "allocation": .5}]},
])
...
def test_normalized_rows_and_metric_policies():
    with pytest.raises(ValidationError):
        UnitRow(unit_id="u", variant="treatment", metrics={"value": 1.0}, denominators={"value": 0.0})
    with pytest.raises(ValidationError):
        Metric(key="value", name="Value", winsor_upper_percentile=.2)
    assert ExperimentCreate(key="demo", name="Demo").design.alpha == .05


def test_canonical_hash_and_logging_redaction():
    assert content_hash({"a": 1, "b": 2}) == content_hash({"b": 2, "a": 1})
    result = redact_unit_ids(None, "info", {"unit_id": "private", "unit_ids": ["other"], "event": "assigned"})
