...
def test_health_validates_units_and_reports_missingness():
    ...
    assert any(h["status"] == "block" and h.get("metric_key") == "value" for h in check_health(design, rows))
...
def test_proportions_crosscheck_statsmodels():
    ...
    assert result["se"] == pytest.approx(math.sqrt(.4 * .6 / 1000 + .46 * .54 / 1000))
...
@given(st.lists(st.floats(min_value=-1e3, max_value=1e3, allow_nan=False, allow_infinity=False), min_size=2, max_size=80), st.lists(st.floats(min_value=-1e3, max_value=1e3, allow_nan=False, allow_infinity=False), min_size=2, max_size=80))
...
def test_reproduces_pricepoint_power():
    ...
    independent_n = NormalIndPower().solve_power(effect_size=effect / result["standard_deviation"], alpha=.05, power=.8, ratio=1)
    assert math.ceil(independent_n) == 820


@given(st.floats(min_value=.005, max_value=.3), st.floats(min_value=.6, max_value=.95))
def test_sample_size_monotone(mde, requested_power):
    base = power.required_sample_size(10, mde, power=requested_power)
    assert power.required_sample_size(10, mde * 1.2, power=requested_power)["n_per_arm"] <= base["n_per_arm"]
    assert power.required_sample_size(10, mde, power=(requested_power + 1) / 2)["n_per_arm"] >= base["n_per_arm"]


def test_power_calculator_allocation_and_inverse():
    planned = power.required_sample_size(10, .04, standard_deviation=3, allocation=.3, daily_units=300)
    assert planned["n_treatment"] == math.ceil(planned["n_control"] * .3 / .7)
    assert planned["days"] > 0
    inverted = power.minimum_detectable_effect(884, 10, standard_deviation=3)
    assert inverted == pytest.approx(.04, abs=.0001)
    for bad in [{"baseline": 0}, {"allocation": 0}, {"daily_units": -1}, {"kind": "ratio"}]:
        with pytest.raises(ValueError):
            power.design_power({"baseline": 10, "mde_relative": .04, **bad})
    with pytest.raises(ValueError):
        power.required_sample_size(.99, .2, kind="proportion")


def test_cuped_zero_covariance_returns_unadjusted():
    c, t, pre = np.array([1., 2., 1., 2.]), np.array([2., 3., 2., 3.]), np.array([-1., -1., 1., 1.])
...
def test_guardrails_direction_and_noninferiority():
    ...
    assert not guardrails.evaluate(metric, {"guardrail_margin_relative": .02})["passed"]
    assert guardrails.evaluate(metric, {"guardrail_margin_relative": .02, "direction": "lower_is_better"})["passed"]
    with pytest.raises(ValueError):
        guardrails.evaluate(metric, {})


@pytest.mark.parametrize("interval,guardrails_list,expected", [((.1, .7), [], "ship"), ((-.1, .2), [], "no_ship"), ((-.1, .6), [], "extend"), ((.1, .7), [{"passed": False}], "no_ship")])
def test_decision_rule_branches(interval, guardrails_list, expected):
    primary = {"ci_low": interval[0], "ci_high": interval[1], "estimate": sum(interval) / 2, "n_control": 200, "n_treatment": 200}
    assert decide(design_for("at_mde"), primary, guardrails_list, [{"status": "pass"}])["decision"] == expected
...
def test_lower_is_better_decision_and_missing_primary():
    ...
    primary = {"ci_low": -.7, "ci_high": -.1, "estimate": -.4, "n_control": 200, "n_treatment": 200}
...
def test_outlier_policy_recorded_and_no_preperiod_or_time_is_honest():
    ...
    design["metrics"][0]["winsor_upper_percentile"] = .99
...
def test_srm_thresholds_match_chisquare():
    ...
    with pytest.raises(ValueError):
        srm(observed, {"control": .2, "treatment": .2})
