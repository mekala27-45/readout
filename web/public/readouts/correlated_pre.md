...
Source: **simulated**. A pre-period covariate predicts outcome variation; theoretical reduction is rho squared.
Scenario: `correlated_pre`. Seed: 20260922.

Analysis recorded: 2026-09-22T15:57:14.720956+00:00. Input fingerprint: `170b2cecd3c419bc070d55c0375a44c0530a1fac3bb0deac7189bba79054590c`.
...
| --- | --- | ---: | ---: | --- |
| srm_assigned | pass | 3.2013 | 0.073578 | treatment is short by 49.0 units versus configured allocation; block below 0.001, warn below 0.05. |
| srm_exposed | pass | 3.2013 | 0.073578 | treatment is short by 49.0 units versus configured allocation; block below 0.001, warn below 0.05. |
...
| --- | ---: | ---: |
| control | 1549 | 1,500.0 |
| treatment | 1451 | 1,500.0 |
...
A pre-period covariate predicts outcome variation; theoretical reduction is rho squared.
...
| --- | --- | --- | ---: | ---: | ---: |
| Value per exposed unit | 0.4442 [0.2275, 0.6610] | 4.45% [2.23%, 6.67%] | 6.01e-05 | 1549 | 1451 |

Standard error: 0.1106. Method: Welch t.
Projected power at the registered effect and achieved sample: 95.44%. This is a design projection, not observed-effect evidence.
Unit bootstrap cross check: 0.4442 [0.2327, 0.6645]. Resamples: 400; seed: 20260922.

## Guardrails

| Metric | Absolute effect and interval | Relative effect and interval | Margin | Verdict | One-sided p-value |
| --- | --- | --- | ---: | --- | ---: |
| Quality score | -0.3843 [-1.1022, 0.3335] | -0.38% [-1.10%, 0.33%] | 5.00% | pass | 3.69e-37 |
...
| --- | --- | ---: |
| Unadjusted | 0.4442 [0.2275, 0.6610] | 0.1106 |
| CUPED adjusted | 0.4679 [0.3371, 0.5987] | 0.0667 |

Descriptive variance reduction: 63.59%. Equivalent sample multiplier: 2.75. Equivalent units saved: 1,908. These are variance-model planning equivalents, not additional observed users. The decision uses the registered unadjusted primary. Same-sample theta has small finite-sample bias; pre-period covariates must be unaffected by treatment.
...
First sequential rejection day: 14. First naive rejection day: 11.

| Final day | Method | Effect | Current interval | p-value |
| ---: | --- | ---: | --- | ---: |
| 30 | Sequential monitor, running-min p | 0.4442 | [0.1071, 0.7813] | 0.002077 |
| 30 | Naive daily comparison | 0.4442 | [0.2275, 0.6609] | 5.87e-05 |
...
| --- | --- | ---: | ---: | --- | --- |
| Retention | 0.0213 [-0.0138, 0.0565] | 0.234514 | 0.234514 | not significant | not significant |
| Segment desktop: Value per exposed unit | 0.4138 [-0.0090, 0.8365] | 0.055042 | 0.110083 | not significant | not significant |
| Segment mobile: Value per exposed unit | 0.2329 [-0.2005, 0.6663] | 0.291742 | 0.291742 | not significant | not significant |
| Segment new: Value per exposed unit | 0.8120 [0.3571, 1.2669] | 0.000486 | 0.001944 | significant | significant |
| Segment returning: Value per exposed unit | 0.3303 [-0.0972, 0.7579] | 0.129740 | 0.172987 | not significant | not significant |
...
```sh
uv run readout render correlated_pre
```
